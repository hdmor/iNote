...
class NotesViewModel @Inject constructor(private val noteUseCases: NoteUseCases) : ViewModel() {
    ...
    fun onEvent(event: NotesEvent) {
        when (event) {
            ...
            NotesEvent.ToggleOrderSection -> _state.value = state.value.copy(isOrderSectionsVisible = !state.value.isOrderSectionsVisible)
        }
    }

    private fun getNotes(noteOrder: NoteOrder) {
        // for not launching new coroutine when each calling, we must to cancel the job.
        getNotesJob?.cancel()
